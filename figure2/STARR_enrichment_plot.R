...
setwd("/analysis/lixf/tracks/STARR/several_cellLine/11-annoEnrichment_family_subfamily/subfamily_enrichment//")
...
####### filter out copies < 5, filter Promoter intron ..., only remain repeats, plot log2 Ratio (obs/exp)
...
topL1 <- x %>% filter(family == "L1") %>% slice_min(`LogP enrichment (+values depleted)`, n = 40) %>%   pull(Annotation) %>% unique()
...
ggsave("xaxis_numberPeaks_enrichment_changeLTRcolor.pdf")
