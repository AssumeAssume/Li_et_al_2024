...
makeBAM <- function( exp.name, BAM.F, NCORES, Bowtie2Folder, genome, txt.tmp, log.path, bowtie.log.path, bamFile, map.unique, readsQual=0 ) {
...
getUniqueFragends <- function( fragsGR_Unique, firstcutter_Unique="GATC", secondcutter_Unique="GTAC", captureLen_Unique=50, nThreads_Unique=4, baseFolder_Unique, Bowtie2Folder, assemblyName="mm9", config_genomes ) {
  ...
  if ( paste0( "len", captureLen_Unique ) %in% colnames( elementMetadata( fragsGR_Unique ) ) == FALSE ){
    ...
    system( paste0( "samtools view ", bamFile, " | cut -f 1 > ", uniquesFile ) )
...
getFragMap <- function( vpChr_FragMap=NULL, onlyUnique="FLASE", firstcutter_FragMap="GATC", secondcutter_FragMap="GTAC", genome_FragMap="hg19"
...
getVPReads <- function(rds,vpRegion=2e6) {
  ...
  peakCDat <- data.frame(pos=vpGR$pos,reads=vpGR$norm4C)
